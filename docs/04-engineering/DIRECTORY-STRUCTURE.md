...
├── configs/
│   └── game-data/                    # 见 CONFIG-SCHEMA.md
├── prisma/
│   └── schema.prisma                 # 数据库模型定义,必须与 DATA-MODEL.md 保持一致
├── docs/                              # 本文档体系
├── docker-compose.yml
├── pnpm-workspace.yaml
└── package.json
...
| 目录                      | 允许做的事                                                           | 禁止做的事                                                                      |
| ------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `apps/web`                | 页面渲染、用户交互、调用后端 API、展示后端返回的分数                 | 自行计算综合评分;直接读取 `configs/game-data` 文件                              |
...
| `configs/game-data`       | 存放版本化的装备/干员/权重数据                                       | 被业务代码直接硬编码引用具体数值                                                |
