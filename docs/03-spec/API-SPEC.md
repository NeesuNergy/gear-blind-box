...
- `configVersion` 是官方 `config_revisions.versionTag`(单例,见 `DATA-MODEL.md` 2.3 节 / ADR-0002),四类装备与权重共用同一个全局版本标签,不再按类别分别维护版本号。
- `scoreBounds` 是当前配置下理论可达成的综合评分最小/最大值,前端用于限制用户设定区间滑块的边界,避免用户设定一个必然 `infeasible` 的区间(体验优化,后端仍需做兜底校验);计算口径见 `SCORING-SPEC.md` 第 3.3 节。
...
| 2026-07-22 | v0.2 | 修正 3.1 节 `configVersion` 示例:改为单一字符串(官方 `config_revisions.versionTag`),对齐 ADR-0002 / `DATA-MODEL.md` v0.3;此前示例遗留自按类别多 JSON 文件版本化的旧方案,未随迁移同步更新 |
