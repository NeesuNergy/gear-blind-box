...
- 四项均在 `[0, 1]`(闭区间,允许取 `0`)。
- **评分中立槶位**:若某个槏位(如干员)被认为没有强弱区分、不应影响综合评分,将其权重设为 `0` 即可——`weight × baseScore` 恒为 0,该槏位不再影响总分,也不会被评分区间预筛剔除,但仍按各条目的 `GearItem.weight`(抽取权重,与此处的评分权重是两个独立字段)正常参与随机抽取。**不要**通过"把该类别所有 `baseScore` 填成同一个值"来模拟这个效果——那样仍会占用权重预算、压缩其余槏位的评分区分度,且高度依赖人工维护同步,容易在新增条目时被悄悄破坏。
...
| 2026-07-23 | v0.4 | 补充"评分中立槶位"用法说明(权重设 0);官方权重调整为干员不参与评分(见 seed-data) |
