...
### 3.3 辅助工具:分数边界计算(scoreBounds)

`GET /api/v1/gear-config`(见 `API-SPEC.md` 3.1 节)需要返回当前有效池下理论可达成的综合评分 `[min, max]`,用于前端限制区间滑块边界。计算口径:

```
minScore = round(Σ weight_i * min(pool_i.baseScore))
maxScore = round(Σ weight_i * max(pool_i.baseScore))
```

即每个槶位独立取该槶位候选池内的最低/最高 `baseScore`,按权重求和后四舍五入。这与第 4.2 节"预筛"中每个槶位的 `minPossibleContribution` / `maxPossibleContribution` 是同一概念,`scoreBounds` 即为四个槶位该数值的汇总,供展示层使用,不参与抽取时的约束传播计算(约束传播按槶位单独判断,见 4.2 节)。

### 3.4 扩展位:协同加成(Synergy Bonus)
...
| 2026-07-22 | v0.2 | 新增 3.3 节 scoreBounds 计算口径,供 `API-SPEC.md` gear-config 接口使用 |
